...
#         urdf_path = './urdf/iiwa.urdf'
    ...
    def get_IK(self, endEffectorPos):
        numJoints = p.getNumJoints(self.robotId)
        
        # set the desired end effector orientation
        endEffectorOrientation = p.getQuaternionFromEuler([0, np.pi, np.pi/2])

        # use PyBullet solve inverse kinematics for joint angles
        jointAngles = p.calculateInverseKinematics(self.robotId, 7, endEffectorPos, endEffectorOrientation, 
                                                   jointDamping=[0.01]*numJoints, solver=p.IK_DLS)
